...
#         pi = 4 / (num * (num+1) * (num+2))*(1)-4 / (num * (num+1) * (num+2))*(-1)+pi
#         
#     return pi
...
lst_mnogo4 = [randint(0, 100) for x in range(0, N)]
...
for i in range(0, len(lst_new)):
    if lst_new[i][:lst_new[i].find('*')] == '0':
        lst_new.remove(lst_new[i])
    if lst_new[i][lst_new[i].rfind('*'):] == '*1':
        lst_new[i]=lst_new[i].replace('**1','')
    if lst_new[i].startswith('1*'):
        lst_new[i]=lst_new[i].replace('1*', '')
    

result = f'{" + ".join(lst_new)} + {randint(0, 100)} = 0'
with open("resul.txt",'w') as file:
    file.write(result)
